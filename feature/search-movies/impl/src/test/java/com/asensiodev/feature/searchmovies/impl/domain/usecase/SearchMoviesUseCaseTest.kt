...
class SearchMoviesUseCaseTest {
    ...
    fun `GIVEN refresh request WHEN refresh THEN repository uses refresh policy`() =
        runTest {
            every { repository.searchMovies("inception", 1, FetchPolicy.REFRESH) } returns
                flowOf(Result.success(emptyList()))

            useCase.refresh("inception", 1).toList()

            verify(exactly = 1) {
                repository.searchMovies("inception", 1, FetchPolicy.REFRESH)
            }
        }
}
